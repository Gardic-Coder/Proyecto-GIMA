...
export const articuloService = {
   ...
   */
  async create(token: string, data: { tipo: string; marca: string; modelo: string; descripcion: string }) {
   ...
   */
  async update(token: string, id: number, data: { tipo?: string; marca?: string; modelo?: string; descripcion?: string }) {
  ...
  async delete(token: string, id: number) {
    ...
    if (!response.ok) {
        ...
        const message = text ? JSON.parse(text).message : "Error al eliminar la categoría";
